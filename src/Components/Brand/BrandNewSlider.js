...
export default function BrandNesSlider() {
  ...
  const fetchBrandNewData=async()=>{
    let list = await getData('brand/fetchnewbrandadlist')
    setBrandNewList(list)
  }

 const BrandNewSlider=()=>{
   return(
    getNewTopList.map((item)=>{
      return (
    
    <div>
        <div className={classes.cardroot} >
           <Avatar variant="rounded" className={classes.media} 
              alt={item.categoryname} src={`${ServerURL}/images/${item.ad}`}
           />
